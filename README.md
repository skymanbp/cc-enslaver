...
[![Plugin Version](https://img.shields.io/badge/version-0.23.0-blue.svg)](CHANGELOG.md)
...
└── tests/                       # 310 black-box + unit tests (run with python -m unittest discover tests)
```
...
- **`lib/state.py`** + **`lib/edicts.py`** + **`lib/sync_gate.py`** — shared per-session-state library; Imperial Edicts loader / matcher / **multilingual renderer** (English default; `zh` — or any code — via `CC_ENSLAVER_LANG`, with English fallback for unknown codes; v0.17 + v0.21); and the rule-12 sync-gate config loader / evaluator (v0.23).

All scripts are covered by **310 tests** in [`tests/`](tests/) (black-box subprocess tests for the hooks + unit tests for `lib/sync_gate.py`) — run with `python -m unittest discover tests`. CI matrix: ubuntu-latest × windows-latest × Python 3.13. Session state is safe under parallel hook processes (v0.23): every mutation holds a cross-process file lock and saves atomically, pinned by a 12-way concurrency regression test. Production payload shapes (no `turn_count`, transcript-only Stop messages) are pinned by a dedicated E2E test class.
