...
- **English（骨架 / source of truth）** — [`../rules/01-*.md` ~ `../rules/09-*.md`](../rules/)（root 层）。英文是**骨架语言**：钩子注入默认英文（`prompts/` root），任意其它层的规则语义都以英文骨架为准。命令 / skill 的**正文**用中文书写（作者语言，可以是任意语言），但它们引用的规则**定义**以英文骨架为准。
