...
| [`../hooks/scripts/lib/messages.py`](../hooks/scripts/lib/messages.py) + `messages_en.py` / `messages_zh.py` | **所有规则的输出前提**（v0.38）：守卫**打印**的每一句话（deny 理由、九层状态表、恢复指引）都住在消息目录里，按 `CC_ENFORCER_LANG` **逐键**解析、缺键回落英文骨架。此前这些文案硬编码在守卫里且是**双语混排**的——英文正文缀一行中文 `大白话`——而 `CC_ENFORCER_LANG` 只管注入文案、管不到它们。规则的**判定**不变，变的是这些判定用哪种语言讲给你听 |
| [`../hooks/scripts/lib/hookio.py`](../hooks/scripts/lib/hookio.py) | **所有规则的输入前提**（v0.37）：四个钩子入口统一从 `sys.stdin.buffer` 读字节、按 UTF-8 严格解码。此前走 `sys.stdin.read()`（宿主机码页 + `surrogateescape`），非 UTF-8 宿主机上载荷被静默改写——规则 09 的 rationale 判定被一个破折号推翻，而规则 01/06/07/08/12 在 Stop 层要找的**中文**标记全成乱码，等于中文回复整轮不受管。它决定的不是某一条规则，而是每条规则**是否看得见它要判的那串字** |
