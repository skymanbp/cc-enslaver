...
| 09 | Systematic modification / no patch-style | Patch markers require a why-comment adjacent; no rolling patches; no wrapping the call site to make exceptions vanish. **Bulk edits (v0.22.1):** a rename / codemod / sed needs a survey of the token's real neighbourhoods → an allowlist → a refusal report → reconciled arithmetic; never rewrite a path handed to a fixed git rev; enumerate the legal set instead of blacklisting stray shapes. **Unified fix (v0.28):** point-to-point patching is banned — a diagnosed root cause defines a *class*; sweep every sibling instance and ship ONE systematic change (N symptoms with one root = one fix, never N patches; class coverage itself is text-level — no hook can verify it). Violation → Stop **layer (f)** BLOCK. |
...
- About to fix the *second* failure of the same shape one-by-one, without diagnosing the shared origin and sweeping its class → rule 03 upstream ladder + rule 09 unified fix (v0.28)
