...
标量摘要没动，**不是**"什么都没变"的证据。总数、通过数、文件大小、
问题条数完全可以在下面的**构成**翻天覆地时纹丝不动。

- **比集合，不要比计数。** 类别名、测试 ID、每条失败断言的身份、
  逐文件哈希——这些才是对照物。`N` 只是你真正关心的东西的有损投影。
- **真实案例。** 一个校验器在约 9,500 处替换的重构前后都打印
  `Total issues: 754`，逐位相同；而逐类别比对显示有一类从
  `OK …: INFO:1` 翻成了 `X …: CRITICAL:1`。只比总数就会把这条
  CRITICAL 当成"没变化"发出去。
- **测试同理。** 记录**失败测试 ID 的集合**，不是"N 个失败"。
  `N` 不变而成员不同 ＝ 一个回归和一个修复在摘要行里互相抵消了。
- **证据的覆盖面 ≠ 结论的覆盖面。** 当自动门只校验制品的一部分时，
  它变绿这件事对其余部分**什么也没说**。声称"整个制品完好"之前，
  必须人工逐条核对门管不到的那部分。（实例：一道计划替换门只守
  `steps` 列表，顺利放行，而未被守护的 `success_criteria` 里
  静默掉了两条。）
...
- ❌ "总数一样就是没变" —— 标量相同不等于集合相同（验证 2b）。
- ❌ "门是绿的就说明制品完好" —— 门只对**它检查的那部分**是绿的，其余未经验证（验证 2b）。
...
- 即将凭一个**总数相同**（而非逐项集合比对）声称"没变 / 中性 / 无回归";
- 某道自动门变绿，而你正要把这个结论推广到它并不检查的部分;
- 自答 4 题中任意一题用了模糊词。
...
2b. 验证 2b —— 任何"没变"的声称都建立在集合比对而非标量之上；自动门覆盖不到的部分已人工核对；
3. 验证 3（连带）已运行，无新增失败；
