...
| **Edit/Write 频率层**（v0.13） | `PreToolUse(Edit\|Write)` | 同一文件本会话第 4 次"小幅 Edit" (≤ 10 行 且 < 200 字符) 而无系统式重写（≥ 50 行 / ≥ 1500 字符 / **≥ 文件的 30%**）介入。**永不计数**（v0.35）：净减少改动、记账类改动 | **DENY** |
...
| **systematic** | max 字符 ≥ 1500 **或** max 行数 ≥ 50 **或** 改动跨越目标文件的 ≥ 30%（任一轴，v0.35） | 重置为 0 |
| **medium** | 介于两者之间 | 不动 |
...
只有绝对下限时，计数器在**小文件上是不可恢复的**。一个不足 1500 字符、
不足 50 行的文件，任何改动都够不到 "systematic"——把一个 30 行模块整体
`Write` 一遍会被判为 *medium*，既不计数也不重置——于是三次小改之后该文件
本会话被锁死，唯一合法出路是把文件**撑到** 1500 字符以上。一个反对反应式
打补丁的闸门，要求你把文件写得更长。

因此覆盖率是**额外**的达标通道，而不是替换：此前算 systematic 的改动现在
仍然算。两端一起缩放（`max(1500, 文件的 30%)`）的方案被否决，因为它会抬高
大文件的下限，在另一端复制出同一个锁死缺陷。

代价明说而非隐藏，并给出实测数字：本层只在**约五行及以下**的文件上失效——那时
一次两行的改动就已跨越文件的三分之一。从六行起，绝对的 small 定义仍然生效：一个
30 行的文件照样会拒绝它的第四次两行小改，因为它的覆盖率门槛是 10 行。这是有意的
——*"你没有重新理解整个文件结构"* 这句话，对一个五行的文件本来就不成立。

#### 两类改动永不计数（v0.35）

| 豁免类型 | 判定 | 为什么它不可能是滚动补丁 |
|---|---|---|
| **净减少** | `len(new) < len(old)` | 滚动补丁的本质是小幅**累加**。让文件比原先更短的改动，正是本层要制止的行为的反面。 |
| **记账类** | 抹掉数字串后两侧逐字节相同，且每个变动的数字位在两侧都是**同一种记账形状**——版本号（`1.2.3`）或 ISO 日期（`2026-08-25`）；散文档里还包括纯整数 | 升版本号 / 改日期不是症状修复。 |

记账白名单是**按形状而非"任何数字"**，恰恰因为 `timeout = 30 → 300`、
`if n > 3 → n > 4` 同样是小幅数字改动——而本规则明令禁止拉长 timeout 与放宽
断言。纯整数只在两者都不存在的地方豁免：散文档，即规则 10 / 11 已经豁免的
那一集合。

两个豁免只作用于**频率层**。内容检测器先于它运行且不受影响：一次删掉五十行、
顺手塞进一个无理由 `# noqa` 的改动照样被拒——压制与文件往哪个方向增长无关。

DENY 时给出的恢复路径：
1. 合并多个待办小改成一次系统式 Edit（new_string ≥ 50 行 / ≥ 1500 字符 / ≥ 文件的 30%）；
2. 用 `Write` 整体覆写；
...
- ❌ **滚动补丁**：同一文件本会话 ≥ 4 次小幅 Edit 而**没有**一次系统性重写，属于反应式累加。v0.13 起由 `PreToolUse(Edit|Write)` 频率层物理拦截，不再是软纪律。净减少与记账类改动豁免（v0.35）——两者都不累加。
