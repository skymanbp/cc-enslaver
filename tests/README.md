...
**746 tests, 21 files, zero dependencies.** This file is the index: every
...
| File | Tests | Covers |
|---|---:|---|
| [`test_inject_context.py`](test_inject_context.py) | 32 | [`inject_context.py`](../hooks/scripts/inject_context.py) — SessionStart / UserPromptSubmit payload shape, language switching, UTF-8 + CJK survival, the YAML reply-schema contract, the 10,000-character output cap, and the v0.34.1 edict-clipping regression (boundary pattern coupled to the real rendered row shape; elision notice must report the true count). **v0.38.3 + v0.39 add the budget half**, all of it found by cloning to a long path rather than by reading the code: a total elision must still report its count (the v0.34.1 defect in the sibling branch), the header must name the install root once (the repeat was spent from the budget it protects), and a 120-character root plus three real edicts must survive in both languages — the gate nothing had, while the contract ate 439 characters of its own headroom across nine releases. The two elision cases build **synthetic** bodies sized to the cap rather than reading the live prompt: pinned to the real contract they silently stop reaching their branch every time it is edited, which is what v0.39's thinning did to them. |
...
|---|---:|---|
| [`test_version_sync.py`](test_version_sync.py) | 9 | A version pointer that does not match `plugin.json`. The set of version-bearing JSON pointers is *closed*, so a newly added field fails until registered. Born from v0.22.1, which shipped with a stale `marketplace.json`. **Also a released heading with no tag:** every `## [x.y.z]` is compared against `git tag`, not just the newest one, so a version that got its entry and its `release:` commit while the tag was never cut is named instead of staying invisible for every release after it. A version that ships inside another one is registered in `UNTAGGED_BY_RECORD`, and the registration only holds while its own entry says there is no separate tag. **The gate's own input is pinned too:** `actions/checkout` is depth-1 by default and brings no tags, so `fetch-depth: 0` in the workflow is what the tag half reads through — without it `_git_tags` answers None, the check skips itself, and CI stays green over a gate that inspected nothing. |
| [`test_doc_sync.py`](test_doc_sync.py) | 23 | Stale counts and inventories: rule count, command count, test count, the Bash deny set on every surface that claims to list it, the `lib/` module inventory in all three structure trees (README.zh.md joined in v0.34.1, closing the mirror-coverage class for the third time), and every repo-relative markdown link. **v0.35.1 adds three gates over things a doc can only get wrong by hand:** advertised hedge triggers are re-derived from `stop_guard._HEDGE_INNER` (both READMEs named `should be fine` / `应该`, which the detector deliberately ignores — so the layer-(b) demo's output could not have come from its own input); sample coverage bars are re-derived from `editscale.coverage_bar` (the sample printed `1104` where the code computes `1102`); and every backticked `UPPER_SNAKE` identifier in a non-CHANGELOG doc must resolve to a real Python definition or be registered in `DOC_ONLY_IDENTIFIERS` with a reason (ARCHITECTURE cited four block-reason constants deleted in v0.12.0 for the twenty-three releases since). All three were checked RED against the pre-fix tree before being committed. |
